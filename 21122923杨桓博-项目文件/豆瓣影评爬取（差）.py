...
def get_movie_review_bad():
    ...
    yingping =[]
    response_comment = requests.get("https://movie.douban.com/subject/"+str(sid)+"/reviews?rating=1", headers = headers)
    ...
    #qprint(review_clean)
    with open('网站/static/yingping_bad肖申克的救赎.txt', mode='a', encoding='utf-8', newline='') as file:
            file.write(review_clean)
    with open('D:\wangzhan\网站\static\yingping_cha_wangye'+str(movie_name)+'.txt', mode='a', encoding='utf-8', newline='') as file:
            file.write(review_clean)
    #print(yingping)

# 测试函数
get_movie_review_bad()